...
test('the chip counts what is missing, and retires the count at zero', () => {
  assert.equal(keySetupChipLabel({ setCount: 0, total: 8 }), 'Data sources · 8 keys waiting');
  assert.equal(keySetupChipLabel({ setCount: 7, total: 8 }), 'Data sources · 1 key waiting');
  assert.equal(keySetupChipLabel({ setCount: 8, total: 8 }), 'Data sources · all set');
  assert.equal(keySetupChipLabel(null), 'Data sources · all set', 'no status is not a broken label');
});
