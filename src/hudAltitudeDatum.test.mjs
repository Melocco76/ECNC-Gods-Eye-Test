...
function installHudEnvironment() {
  const elements = new Map(
    ['hud-alt', 'hud-summary', 'hud-mgrs', 'hud-latlon', 'hud-mode']
...
test('the corner ALT readout prints the MSL height, never the ellipsoidal one', () => {
  ...
  assert.equal(
    has(/`Altitude \$\{Math\.round\(altMslM\)\.toLocaleString\('en-US'\)\} m/),
    true,
    'the #hud-alt line must print altMslM',
  );
  assert.equal(
    has(/`Altitude \$\{Math\.round\(altM\)/),
    false,
...
test('the HUD no longer carries simulated sensor math', () => {
  // The GSD/NIIRS/view-band readouts were cosmetic sensor theatre and are gone;
  // only real camera telemetry remains.
  assert.equal(has(/_viewBand/), false, 'the fake view band must stay removed');
  assert.equal(has(/NIIRS/), false, 'the fake NIIRS readout must stay removed');
});
...
test('a cold tick paints both readouts uncorrected, and resolving flips both in one tick', async () => {
  ...
  try {
    ...
    assert.match(alt(), /^Altitude -15 m/, `cold corner readout, got ${alt()}`);
    assert.match(summary(), /Altitude -15 m/, `cold summary tag, got ${summary()}`);
    ...
    hud._updateCameraData();
    assert.match(alt(), /^Altitude 17 m/, `corrected corner readout, got ${alt()}`);
    assert.match(
      summary(),
      /Altitude 17 m/,
    ...
    hud._updateCameraData();
    assert.match(alt(), /^Altitude 17 m/);
    assert.match(summary(), /Altitude 17 m/);
    assert.equal(
...
test('the corrected readouts are the MSL datum, not a coincidence of the SFO sign', async () => {
  ...
  try {
    ...
    assert.match(env.elements.get('hud-alt').textContent, /^Altitude 100 m/);
    await ensureGeoidReady();
    await Promise.resolve();
    hud._updateCameraData();
    assert.match(
    ...
    assert.match(env.elements.get('hud-summary').textContent, /Altitude 54 m/);
  } finally {
    hud?.destroy();
    env.restore();
  }
